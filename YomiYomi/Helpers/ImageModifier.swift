...
struct ZoomableContainer<Content: View>: View {
    ...
    fileprivate struct ZoomableScrollView<InnerContent: View>: UIViewRepresentable {
        ...
        init(scale: Binding<CGFloat>, tapLocation: Binding<CGPoint>, @ViewBuilder content: () -> InnerContent) {
            _currentScale = scale
        ...
        func makeCoordinator() -> Coordinator {
            return Coordinator(hostingController: UIHostingController(rootView: content as! Content), scale: $currentScale)
        }

        func updateUIView(_ uiView: UIScrollView, context: Context) {
            // Update the hosting controller's SwiftUI content
            context.coordinator.hostingController.rootView = content as! Content
